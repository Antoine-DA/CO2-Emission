...
pages= ['👨‍💻Contexte',"🖼️ Cadre de l'analyse des données",'🧹 Nettoyage des données','📈 Data Visualisation', '🏭 Pre-processing','🤖 Modélisation','📚 Synthèse','🚘 Démo']
...
transports. Parmi ces émissions, environ **72%** sont attribuées au **transport routier**, ce qui inclut les
...
if page == pages[3]:  #Data Visualisation
    ...
    colonnes_disponibles = ['ec (cm3)','m (kg)', 'ep (KW)', 'Fuel consumption ']
...
""")
        st.image('image/feature_importance.png')
...
if page == pages[7]: #Démo
    ...
    with st.form(key='prediction_form'):
        ...
        masse = st.number_input("Poids du véhicule (en Kg)",min_value=500.0, max_value= 5000.0, value = 1151.0, step=50.0)
        ...
        capacite_moteur = st.number_input("Capacité du moteur (cm3)",min_value=500.0, max_value=5000.0,value=1199.0,step=50.0)
        puiss_moteur = st.number_input("Puissance du moteur(KW)", min_value=5.0, max_value=1200.0, value=100.0, step=10.0)
        reduc_emission = st.number_input("Technologie de réduction d'émission (g/km)",min_value=0.0, max_value=10.0, value=0.0, step = 0.1)
        consommation = st.number_input("Consommation du moteur (l/100)",min_value=0.0, max_value=30.0, value=5.6, step=0.1)
